...
Sources: `ShorECDLP/Submission/Naive/Arithmetic/ModMul.lean`,
...
### Reachable full-schedule adaptive refinement

The four phase intervals compose on their actual intermediate states. Reachability discharges their arithmetic and window bounds, and induction over Euclidean iterations establishes cleanup conditions throughout the active prefix. The terminal trace supplies the remaining padding steps. `secp256k1EEAForward_adaptiveInput` therefore covers all 1,620 steps under the original clean, nonzero canonical-input assumptions. `secp256k1EEAForwardAdaptive_coherent_preprocessed` derives coherent refinement on every such preprocessed state with one input-independent list of branch coefficients. Composing preprocessing and the output wrapper with this complete adaptive schedule remains subsequent work; no gate streams or resource counts change.

### Complete adaptive forward wrapper

`secp256k1EEAForwardWrapper` composes the pinned preprocessing, all 1,620 adaptive steps, canonical rotation, epoch compression, parity correction and known Work1 clearing. Its coherent refinement applies to original clean, nonzero canonical inputs and yields the existing `secp256k1EEAOutputIdealState`: the inverse coefficient, cleared Work1/epoch/shared scratch, and retained reversal metadata. Branch coefficients are normalized and input-independent; normalization for preprocessing and parity is derived from their actual well-formed instruments. Physical well-formedness covers the complete wrapper, including the source-ordered rotation stages. The reverse wrapper, Figure 15 multiplication/division and aggregate resource certificates remain subsequent work.
