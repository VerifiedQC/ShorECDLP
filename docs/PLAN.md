...
The complete Figure 15 division and multiplication circuits now each have worst-branch T count 269,605,707 on the same terms as their coherent contracts. Each EEA wrapper costs 123,142,257 T gates, including preprocessing, all 1,620 steps, canonical rotation, epoch compression and parity correction. Together with the previously proved 11,343,835 measurements, this closes numerical T/measurement accounting; aggregate adaptive physical support remains open.

Measurement-uncomputed decoder trees, ripple cells, interval traversals and complete forward/inverse interval circuits now have direct adaptive-wire subset proofs against their coherent references. These certificates add no layout assumptions and cover measurement corrections as well as gates. Propagation through coefficient scans, full indexed steps, wrappers and the aggregate Figure 15 allocation remains open.

The actual adaptive forward EEA now has a direct 580-wire certificate. Coefficient scans and epoch-aware phase tests refine their coherent physical support; the complete forward and inverse indexed steps and schedules inherit that inclusion. The forward allocation follows from the existing coherent production layout, counting every gate and measurement branch. The inverse allocation and complete preprocessing/postprocessing and Figure 15 bounds remain open.

The explicit reverse EEA now also has a direct 580-wire adaptive certificate. Its independently ordered A–H inverse components are checked against the production allocation, then composed over the descending schedule and transferred to all adaptive branches. Complete wrapper and Figure 15 physical bounds remain open.
...
The source inverse square now explicitly runs copied-control subtraction and halving from the least significant input bit upward. It reuses the existing forward square and proves that every forward/inverse measurement-branch pair restores the complete original zero-output state. The same inverse circuit has 1,110,533 CCX, 3,429,962 CX, 7,773,731 T, 261,121 measurements and exactly 517 wires. Its forward, inverse-on-image and composed identity contracts use one normalized branch expansion across all valid inputs, preserving arbitrary supported superpositions. Total controlled point addition, including exceptional inputs, remains open.

The literal five-stage controlled constant modular adder now has generic arithmetic, full-frame, well-formedness and measurement-branch proofs. Its controlled less-than comparator includes the source threshold shortcuts. The same concrete secp256k1 controlled-increment circuit has normalized mass, 3,829 CCX, 26,803 T gates, 1,278 measurements and exactly 517 wires, with its reduction flag and all other non-target wires restored. Remaining Figure 14 composition and total exceptional point cases are still open.

Figure 14's literal square–controlled-subtract–inverse-square block now coherently maps X to X−ctrl·Y² modulo p on canonical inputs with a clean square accumulator. The proof tracks the source helper rotation, restores every wire outside X, and clears the accumulator after the intervening subtraction. The inverse image is reconstructed from the retained square value and clean helpers, without requiring the external X bank to equal its pre-square state. Direct canonical modular subtraction is derived from the existing inverse-after-add theorem. Complete point-addition composition and exceptional cases remain open; no new aggregate resource bound is claimed here.

The source controlled modular negation now proves canonical output for every input, including zero: it marks nonzero input, conditionally complements and increments, adds p under that flag, then clears the flag. All actual measurement branches agree with this arithmetic map and preserve every other wire, including arbitrary borrowed dirty values. A normalized coherent contract covers arbitrary supported superpositions. Full Figure 14 composition, exceptional point cases and Phase 9 aggregate resources remain open.
