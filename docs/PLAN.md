...
Figure 14 squaring now has its copied-control modular-add primitive: copy an addend bit to a separate control, add under that control, then clear the copy. The full-state and quantum branch proofs preserve the original addend and all work. Its actual 256-bit certificate gives 2,813 Toffolis, 4,931 CX, 19,691 T, 511 measurements and at most 517 wires. The complete square/inverse schedule and total point-addition exceptional handling remain open.
