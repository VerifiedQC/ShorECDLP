#!/usr/bin/env python3
"""Independent sparse-state replay of actual Lean lookup/add/clear programs."""
...
def main : IO Unit := do
 for n in [0:2] do
  ...
  let program := lookupModularAddProgram table bits paths ([3,4] : List Wire) ([5,6] : List Wire) (constantBits 2 1) 3 0 7 8 9 10
...
if __name__=='__main__':
    with tempfile.TemporaryDirectory(prefix='lookup-add-') as directory:
    ...
    for line in out.splitlines():
        ...
        basis=[1|(addr<<1)|(acc<<5)|(spectator<<11) for addr in range(2**n) for acc in range(3) for spectator in range(2)]
        states=[{word:1+0j} for word in basis]
        states += [{w:cmath.exp(1j*(i+1))/math.sqrt(len(basis)) for i,w in enumerate(basis)}]
        for state in states:
            expected={}
            for word,amp in state.items():
                address=(word>>1&1) if n else 0
                result=((word>>5&3)+(2*address+1)%3)%3
