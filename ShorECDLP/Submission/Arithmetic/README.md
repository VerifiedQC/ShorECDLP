...
| [`ModAdd.lean`](ModAdd.lean) | Adds two registers modulo an arbitrary positive modulus using ripple addition, one conditional reduction, and uncomputation. | `modAdd`, `modAdd_correct`, `modAdd_clean`, `modAdd_tCount`, `modAdd_contract` |
| [`ModSub.lean`](ModSub.lean) | Subtracts two registers modulo an arbitrary positive modulus using two's-complement ripple subtraction, conditional correction, and uncomputation. | `modSub`, `modSub_correct`, `modSub_clean`, `modSub_tCount`, `modSub_contract` |
