...
private def gidneyRecordedPhase : List Bool → List Bool → ℂ
...
private theorem gidneyBorrowedPhaseCancellation (dirty : List Wire) (outcomes carries : List Bool)
...
private noncomputable def gidneyRawCoefficient : List Bool → List Bool → ℂ
  | [], [] => 1
  | outcome :: outcomes, carry :: carries =>
      Quantum.xResetCoeff outcome carry * gidneyRawCoefficient outcomes carries
  | _, _ => 0

private theorem gidneyRawCoefficient_phase (outcomes carries : List Bool)
...
private theorem gidneyUnitaryBranch (circuit : Circuit) (next : Quantum.AdaptiveCircuit)
...
private theorem gidneyMeasureBranch (target : Wire) (onFalse onTrue : Quantum.AdaptiveCircuit)
...
private theorem gidneyDoneBranch (branch : Quantum.InstrumentBranch)
...
private theorem gidneyCarryBits_length (input constant : List Bool) (carry : Bool)
...
private theorem gidneyXorWord_cancel (bits carries : List Bool) (hlen : bits.length = carries.length) :
...
private theorem gidneyCarryXor_HPFree (input dirty : List Wire) (constant : List Bool)
...
private theorem gidneyGateCount_tCount (program : Quantum.AdaptiveCircuit) :
...
private theorem gidneyZ_cost (cost : Gate → Nat) (hX : ∀ w, cost (.X w) = 0)
...
private theorem gidneyZ_usesOnly (dirty : List Wire) (outcomes : List Bool) :
