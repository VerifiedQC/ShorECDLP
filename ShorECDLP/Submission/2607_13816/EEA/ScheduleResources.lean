...
theorem indexedScheduleInverseAdaptive_measurementCount (r : IndexedStepRegisters) (n start count : Nat)
  ...
  | @step start count head tail ih =>
    ...
    simp only [List.range'_succ, List.map_cons, List.sum_cons]
    omega
theorem dualUnaryAction_leaves_eq_labels_length (t : DualUnaryActionTree) : t.leaves=t.labels.length := by
  induction t with
  | leaf => rfl
  | node a b z o hz ho => simp [DualUnaryActionTree.leaves,DualUnaryActionTree.labels,hz,ho]
theorem dualUnaryAction_internalNodes_add_one (t : DualUnaryActionTree) : t.internalNodes+1=t.leaves := by
  induction t with
  | leaf => rfl
  | node a b z o hz ho => simp only [DualUnaryActionTree.leaves,DualUnaryActionTree.internalNodes]; omega
theorem unaryAction_leaves_eq_labels_length (t : UnaryActionTree) : t.leaves=t.labels.length := by
  induction t with
  | leaf => rfl
  | node a z o hz ho => simp [UnaryActionTree.leaves,UnaryActionTree.labels,hz,ho]
theorem unaryAction_internalNodes_add_one (t : UnaryActionTree) : t.internalNodes+1=t.leaves := by
  induction t with
  | leaf => rfl
...
private theorem interval_leaf_count (r : IntervalRegisters) (k K : Nat) :
  ...
  rw [dualUnaryAction_leaves_eq_labels_length,intervalTree_labels,Finset.length_sort]
  apply List.toFinset_card_of_nodup
  unfold intervalMainLabels
  split <;> exact List.nodup_range
theorem coefficientPrefix_leaf_count (r : CoefficientPrefixRegisters) (k K : Nat)
    (h : CoefficientPrefixLayout r k K) : (coefficientPrefixTree r k K).leaves=K+1-k := by
  rw [unaryAction_leaves_eq_labels_length,coefficientPrefixTree_labels r h,Finset.length_sort]
...
private theorem interval_scalar (r : IntervalRegisters) (k K : Nat) (target : IntervalTarget)
  ...
  have hn := dualUnaryAction_internalNodes_add_one (intervalTree r k K)
...
theorem secp256k1StepMeasurements_eq (T : Nat)
  ...
  let w := certifiedActiveWindows 256 T
  have hc := coefficientPrefix_leaf_count (indexedStepProductionRegisters.coefficient w.coefficient)
    w.coefficient.start w.coefficient.stop h.coefficient
  have hn := unaryAction_internalNodes_add_one (coefficientPrefixTree (indexedStepProductionRegisters.coefficient w.coefficient)
...
private theorem interval_cost_sum (r : IntervalRegisters) (k K : Nat) (target : IntervalTarget)
  ...
  · have hf : intervalHasTopSpecial k K = false := Bool.eq_false_iff.mpr hs
    ...
    simp
theorem dual_nodes_labels (t : DualUnaryActionTree) : t.internalNodes+1=t.labels.length := by
  induction t with
  | leaf => rfl
  | node a b z o hz ho =>
...
theorem productionStepTComponents_eq (T : Nat)
  ...
  let w := certifiedActiveWindows 256 T
  have hc := coefficientPrefix_leaf_count (r.coefficient w.coefficient) w.coefficient.start w.coefficient.stop h.coefficient
  have hq := quotient_leaf_count (r.quotient w.quotientSwap) w.quotientSwap.start w.quotientSwap.stop h.quotient
  have hn := unaryAction_internalNodes_add_one (coefficientPrefixTree (r.coefficient w.coefficient) w.coefficient.start w.coefficient.stop)
  have hm := unaryAction_internalNodes_add_one (quotientSwapTree (r.quotient w.quotientSwap) w.quotientSwap.start w.quotientSwap.stop)
  rw [hc] at hn
  rw [hq] at hm
