...
private theorem sourceTree_routeResidue
    (indexA indexB : Nat → Wire) (tree : DualUnaryActionTree) (labels : Finset Nat)
    (register : List Wire) (second : Bool) (state : BasisState)
    ...
    (hwidth : DualUnaryActionTree.sourceWidth labels ≤ register.length)
    (hvalue : boolWordToNat (wireValues register state) % 2^DualUnaryActionTree.sourceWidth labels ∈ labels) :
    (tree.project second).routeLabel state =
      boolWordToNat (wireValues register state) % 2^DualUnaryActionTree.sourceWidth labels := by
  ...
  · rw [DualUnaryActionTree.buildSource_labels_eq_sort indexA indexB labels tree hbuild]
    simpa using hvalue
  · intro bit hbit
    have hr : bit < register.length := lt_of_lt_of_le hbit hwidth
...
theorem intervalTree_routeResidues (registers : IntervalRegisters) (k K : Nat) (state : BasisState)
  ...
  · exact sourceTree_routeResidue registers.rightIndex registers.leftIndex _ _
      registers.lengthS false state hb (by intro bit; rfl) hrWidth (by simpa using hrValue)
  · exact sourceTree_routeResidue registers.rightIndex registers.leftIndex _ _
      registers.lengthQ true state hb (by intro bit; rfl) hlWidth (by simpa using hlValue)
