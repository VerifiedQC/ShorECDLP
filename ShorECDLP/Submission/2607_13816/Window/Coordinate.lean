...
def lookupCoordinateProgram (x y : Nat → Nat) : AdaptiveCircuit :=
  ((((((((fig14LookupX (fun a => (ShorECDLP.p-x a)%ShorECDLP.p)).seq
    (fig14LookupY (fun a => (ShorECDLP.p-y a)%ShorECDLP.p))).seq
    ...
    (fig14LookupY (fun a => (ShorECDLP.p-y a)%ShorECDLP.p))

def lookupCoordinateState (x y : Nat → Nat) (s : BasisState) : BasisState :=
  let s1 := fig14LookupXState (fun a => (ShorECDLP.p-x a)%ShorECDLP.p) s
  let s2 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s1
  ...
  fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s8
...
theorem lookupCoordinateProgram_coherent (x y : Nat → Nat) :
    CoherentlyImplementsOn (lookupCoordinateProgram x y)
      (Finsupp.lmapDomain ℂ ℂ (lookupCoordinateState x y)) PointLookupValid := by
  ...
  let p2 := fig14LookupY (fun a => (ShorECDLP.p-y a)%ShorECDLP.p)
  let f2 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p)
  have h2 : CoherentlyImplementsOn p2 (Finsupp.lmapDomain ℂ ℂ f2) PointLookupValid := fig14LookupY_coherent _ (fun _ => hk _)
  have r2 : ∀ s, PointLookupValid s → PointLookupValid (f2 s) := fig14LookupYState_ready _ (fun _ => hk _)
  ...
  let p9 := fig14LookupY (fun a => (ShorECDLP.p-y a)%ShorECDLP.p)
  let f9 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p)
  have h9 : CoherentlyImplementsOn p9 (Finsupp.lmapDomain ℂ ℂ f9) PointLookupValid := fig14LookupY_coherent _ (fun _ => hk _)
...
theorem lookupCoordinateState_ready (x y : Nat → Nat) (s : BasisState)
    (hs : PointLookupValid s) : PointLookupValid (lookupCoordinateState x y s) := by
  have hk (k : Nat) : k%ShorECDLP.p<ShorECDLP.p := Nat.mod_lt _ ShorECDLP.Secp256k1.p_prime.pos
  unfold lookupCoordinateState
  apply fig14LookupYState_ready _ (fun _ => hk _)
...
theorem lookupCoordinateState_address (x y : Nat → Nat) (s : BasisState) (hs : PointLookupValid s) :
    tableAddressValue pointLookupAddress (lookupCoordinateState x y s)=tableAddressValue pointLookupAddress s := by
  have hk (k : Nat) : k%ShorECDLP.p<ShorECDLP.p := Nat.mod_lt _ ShorECDLP.Secp256k1.p_prime.pos
  let s1 := fig14LookupXState (fun a => (ShorECDLP.p-x a)%ShorECDLP.p) s
  ...
  let s2 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s1
  have h2 : PointLookupValid s2 := fig14LookupYState_ready _ (fun _ => hk _) s1 h1
  have a2 : tableAddressValue pointLookupAddress s2=tableAddressValue pointLookupAddress s1 := fig14LookupYState_address _ (fun _ => hk _) s1 h1
  let s3 := zeroAllowedDivisionOutputState s2
  ...
  let s9 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s8
  have a9 : tableAddressValue pointLookupAddress s9=tableAddressValue pointLookupAddress s8 := fig14LookupYState_address _ (fun _ => hk _) s8 h8
...
theorem lookupCoordinateState_eq (x y : Nat → Nat) (s : BasisState) (hs : PointLookupValid s) :
    lookupCoordinateState x y s=fig14CoordinateState
      (x (tableAddressValue pointLookupAddress s)) (y (tableAddressValue pointLookupAddress s)) s := by
  have hk (k : Nat) : k%ShorECDLP.p<ShorECDLP.p := Nat.mod_lt _ ShorECDLP.Secp256k1.p_prime.pos
  let s1 := fig14LookupXState (fun a => (ShorECDLP.p-x a)%ShorECDLP.p) s
  have h1 : PointLookupValid s1 := fig14LookupXState_ready _ (fun _ => hk _) s hs
  have a1 : tableAddressValue pointLookupAddress s1=tableAddressValue pointLookupAddress s := fig14LookupXState_address _ (fun _ => hk _) s hs
  ...
  let s2 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s1
  have h2 : PointLookupValid s2 := fig14LookupYState_ready _ (fun _ => hk _) s1 h1
  have a2 : tableAddressValue pointLookupAddress s2=tableAddressValue pointLookupAddress s := (fig14LookupYState_address _ (fun _ => hk _) s1 h1).trans a1
  have e2 := fig14LookupYState_eq (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) (fun _ => hk _) s1 h1.1 h1.2
  rw [a1] at e2
  let s3 := zeroAllowedDivisionOutputState s2
  have h3 : PointLookupValid s3 := lookup_division_ready s2 h2
  ...
  rw [a7] at e8
  let s9 := fig14LookupYState (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) s8
  have e9 := fig14LookupYState_eq (fun a => (ShorECDLP.p-y a)%ShorECDLP.p) (fun _ => hk _) s8 h8.1 h8.2
  rw [a8] at e9
  change s9=_
  dsimp only [s9]
  rw [e9]
...
theorem lookupCoordinateProgram_wires (x y : Nat → Nat) :
    (lookupCoordinateProgram x y).wires ⊆ List.range 855 := by
  unfold lookupCoordinateProgram
...
theorem lookupCoordinateProgram_qubitCount (x y : Nat → Nat) :
    (lookupCoordinateProgram x y).qubitCount ≤ 855 :=
  lookup_count855 _ (lookupCoordinateProgram_wires x y)
