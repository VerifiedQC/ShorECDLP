...
def upd (s : BasisState) (i : Wire) (b : Bool) : BasisState := fun j => if j = i then b else s j

@[inherit_doc] notation:max s:max "[" i " ↦ " b "]" => upd s i b

@[simp] theorem upd_same (s : BasisState) (i : Wire) (b : Bool) : s[i ↦ b] i = b := by
  simp [upd]

theorem upd_other (s : BasisState) (i : Wire) (b : Bool) {j : Wire} (h : j ≠ i) :
    s[i ↦ b] j = s j := by
  simp [upd, h]
