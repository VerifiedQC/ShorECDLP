...
(the standard unitary Toffoli), and a phase rotation `P` costs 1. `tCount` sums this over a
...
/-- Naive T-count of a single primitive gate: Cliffords free, Toffoli 7, phase rotation 1. -/
def tCost : Gate → Nat
  | .X _       => 0
  | .H _       => 0
  | .CX _ _    => 0
