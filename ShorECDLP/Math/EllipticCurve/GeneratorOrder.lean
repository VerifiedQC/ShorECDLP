...
/-!
# secp256k1 generator-order certificate

The fixed Bitcoin primality certificates live in `Math.BitcoinPrimes`.  This shared mathematical
module replays affine scalar multiplication for the published order and generator, then uses the
certified subgroup-order primality and `G_ne_zero` to identify the additive order exactly.
-/
